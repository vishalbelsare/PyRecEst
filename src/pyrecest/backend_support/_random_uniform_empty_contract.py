...
def _patch_random_choice_population_contract() -> None:
    try:
        from pyrecest.backend_support._random_choice_population_contract import (  # pylint: disable=import-outside-toplevel
            patch_random_choice_population_contract,
        )
    except ModuleNotFoundError:  # pragma: no cover - backend support may be unavailable
        return
    patch_random_choice_population_contract()
...
def patch_random_uniform_empty_bounds_contract() -> None:
    ...
    _patch_random_choice_population_contract()
